...
進度: **M1 至 M158 已全數 commit＋push、CI 綠燈**。最終 commit＝HEAD（M158 事件錄音 REST＋SPA（M157 閉環）：GET/POST/DELETE /api/audio、/api/audio/event/{id}＋GET /api/audio?from&to、EventAudioRepository.Delete＋lib audioRows（Storage 812/Api 53/vitest 53）；M157 事件錄音資料面 event_audio（schema v42）：Save/GetByEvent/QueryByTime/PruneOlderThan＋稽核 audio.save，音訊擷取串流屬硬體縫（Storage 811）；M156 智慧牆看板 MQTT 派送掛載 SmartwallBoardDispatcher：SmartwallBoardEvent→{channel_id,event_type,priority,occurred_at_utc,rule_order} JSON PUBLISH 至 helivms/smartwall/alerts/{channelId}，fake IMqttPublisher 離線單測（Storage 806）；M155 智慧大板版面編排 smartwallLayout：rank 排序＋grid 行列分配＋容量截斷，SPA 純 view-model（vitest 52）；M154 排名融合 RankFusion：FTS bm25 與 CLIP 餘弦逐項正規化加權合併＋POST /api/search/fuse＋lib fuseViewRows；M153 CLIP 索引管理 REST：POST /api/clip/index upsert＋GET/DELETE /api/clip/{source}/{refId}；EmbeddingRepository ByRef/Delete；M152 設備管理稽核掛載點：DeviceRepository Add/Update/Delete 全量寫 audit_log（category=config、targetType=device、detail 摘要）＋新增 Update 方法＋GET/POST/PUT/DELETE /api/devices；M151 CLIP 向量語意搜尋管線：EmbeddingRepository（clip_embeddings v41、BLOB float32、餘弦相似 topK）＋POST /api/clip/search＋clipViewRows；M149b 本機攝影機設定讀取器：LocalCameraConfigReader 讀 git-ignored .secrets/cameras.local.yaml、不連網不回寫；M149 磁碟容量趨勢 REST＋SPA：GET /api/system-metrics/disks/history＝SystemMetricsService.CaptureDiskHistory；M148 記憶體趨勢 REST＋SPA：GET /api/system-metrics/history＝SystemMetricsService.CaptureHistory；純 BCL/Storage/Api/SPA 可驗收里程碑已全數交付，剩餘皆 UI/真機/ML 阻塞）。
...
19. **M44 已驗收＝匯出中心（Export Center）：批量匯出＋歷史工作＋完整性驗證（§14.3(2) P0 剩餘）**（commit `99556f6`，全 **283**，CI `35260052774` success；修復 `1112351`＝CI 無 ffmpeg→`ExportJobService` 注入 executor、測試改用 fake executor）：
...
91. **M114 交付＝VMD 靈敏度自動套用（§14.7 #17）**：
    ...
    - 全量牌面（M121 收尾）：Storage 782、Alarms 254、Devices 54、Licensing 8、Api 22＝**1123**（＋vitest 30/12）。
...
123. **M145 交付＝異地備援狀態 REST＋SPA（M55 閉環；僅狀態、未重造複製操作，與 M135 刻意分流）**：
    - WebApi（DI 註冊 OffsiteReplicationRepository＋Service）：`GET /api/replication` → 每工作之來源/目標/間隔/啟用/上次執行/最後結果/錯誤/連續失敗/Due（IsDue）。
    - SPA：admin 異地備援狀態面板（唯讀清單）。lib `repRows`。
    - 測試：Api ＋1 **43**（種子工作→清單含：間隔 60、FAILED、連續失敗 1、LastRunUtc 非空）；vitest ＋1 **44**；全量 **1143**。
122. **M144 交付＝錄影遮蔽日誌 REST＋SPA（M101 閉環；原有 RedactionRepository 僅暴露＋一全域時間查詢，未重造）**：
    - WebApi（DI 註冊 RedactionRepository）：`GET /api/redactions`（sourceType+refId→QueryBySource；channelId→QueryByTime；皆無→QueryByTimeGlobal；limit 1..500）；`POST`（clip/snapshot＋正面積校驗）；`DELETE /{id}`（404）。
    - Storage：RedactionRepository 新增 `QueryByTimeGlobal(from,to)`＋測試。
    - SPA：admin 遮蔽面板——清單＋移除＋新增表單（來源/引用/頻道/XY/寬高/塗滿）。lib `redRows`。
    - 測試：Api ＋1 **42**；Storage ＋1 **784**；vitest ＋1 **43**；全量 **1142**。
121. **M143 交付＝分享連結管理 REST＋SPA（M51 閉環；原有 ShareLinkRepository/ShareToken 僅暴露，未重造）**：
    - WebApi（DI 註冊 ShareLinkRepository）：`GET /api/shares`；`POST`（kind segment/snapshot/evidence＋路徑必填；ShareToken.Create；Label/MaxUses）；`PUT /{id}/revoke`；`DELETE /{id}`；404 守衛；Active 由 revoke/expiry/uses 判定。
    - SPA：admin 分享面板——清單（類型/標籤/資源/權杖 12/用量/狀態）＋撤銷＋建立表單。lib `shareRows`。
    - 測試：Api ＋1 **41**（bad kind 400；建立→Active＋token 非空；清單含；撤銷→revoked 且非 Active；刪除→gone）；vitest ＋1 **42**（shareRows 正常/空）；全量 **1140**。
...
119. **M141 交付＝法律保留 REST＋SPA（M66 閉環；原有 LegalHoldRepository 僅暴露，未重造）**：
    ...
    - 測試：Api ＋1 **39**（窗逆序 400；建立→active；active 清單含；撤銷→active 無、all 含 revokedBy）；vitest ＋1 **40**（holdRows 正常/空）；全量 **1138**。
118. **M140 交付＝登入供應商管理 REST＋SPA（M50 閉環；原有 AuthProviderRepository 僅暴露，未重造）**：
    - WebApi（DI 註冊 AuthProviderRepository）：`GET /api/auth/providers`；`POST`（name＋kind(ldap/oidc)＋合法 JSON 必填；重名→400）；`PUT /api/auth/providers/{id}` 啟停；`DELETE /{id}`；404 守衛。
    - SPA：admin 供應商面板——清單＋啟用核取（就地 PUT）＋刪除＋新增表單（名稱/類型/設定 JSON）。lib `providerRows`。
    - 測試：Api ＋1 **38**（bad kind 400；壞 JSON 400；新增→清單含；toggle off→清單 disable；缺 404；刪除→gone）；vitest ＋1 **39**（providerRows 正常/空）；全量 **1137**。
...
116. **M138 交付＝通知投遞日誌 REST＋SPA（M23 閉環；原有 NotificationLogRepository 僅暴露，未重造）**：
    ...
    - 測試：Api ＋1 **36**（Add webhook/motion ok=1 與 smtp/siren 重試 3 失敗→清單二者齊備；測修正 Add 參數語意＝eventType 首參）；vitest ＋1 **37**（notifRows 正常/空）；全量 **1135**。
...
114. **M136 交付＝門禁事件 REST＋SPA（M92 閉環；原有 DoorEventRepository 僅暴露，未重造）**：
    ...
    - 測試：Api ＋1 **34**（種子兩筆→全列 2、granted=false 過濾 1、card 無命中 0）；vitest ＋1 **35**（doorRows 正常/空）；全量 **1133**。
113. **M135 交付＝異地備援 REST＋SPA（M43 閉環；原有 BackupService/BackupRepository 僅暴露，未重造）**：
    - WebApi（DI 註冊 BackupService＋BackupRepository；改用建構子注入保留 400）：`GET /api/backup/runs`（backup_log 最近 30）；`POST /api/backup/run`（SourceRoot/TargetRoot 必填、來源不存在 400、來源＝目標 400；Run 檢查點推進＋SHA-256 比對）。
    - SPA：admin 備援面板——執行紀錄（#/時間/複製數/位元組/失敗/檢查點/目標）＋執行表單（兩路徑）＋結果訊息（scan/copy/MiB/fail）。lib `backupRows`。
    - 測試：Api ＋1 **33**（缺源 400；種子 final 段＋實際檔→Run 複製→目標檔 SHA 相符→連跑檢查點推進 cop=0→runs ≥2）；vitest ＋1 **34**（backupRows 正常/空）；全量 **1132**。
112. **M134 交付＝證據包 REST＋SPA（M43/M53 閉環）**：
    - WebApi（DI 註冊 EvidenceManifestRepository）：`GET /api/evidence`（manifest 清單＋檔案數）；`POST /api/evidence/package`（BundleName/Files[] 絕對路徑/Password?；檔不存在或空→400；BuildManifest＋Create .evp 至 %TEMP%\\helivms-evidence；Upsert manifest）；`POST /api/evidence/verify`（Verify 回 Valid/Expired/Failures/Items）。
    - SPA：admin 證據面板——清單、打包表單（包名＋路徑多行）、結果訊息（路徑＋sha256 前 12）。lib `evRows`。
    - 測試：Api ＋1 **32**（400 空名/缺檔；打包→檔存在＋Items=1＋sha；驗證 Valid 空 failures；清單含 packaged）；vitest ＋1 **33**（evRows 正常/空）；全量 **1131**。
...
109. **M131 交付＝巡檢 REST＋SPA（M72 巡檢表單封閉）**：
    ...
    - 測試：Api ＋1 **29**（400 空白名→create；steps 存取；缺省 window；PUT 改 window/啟停；DELETE 200→404；PUT 404）＋vitest ＋1 **32**（patrolLabel 正常/空）；全量 **1127**。
108. **M130 交付＝錄影排程 REST＋SPA（M10 排程表單封閉）**：
    - WebApi（DI 註冊 RecordingScheduleRepository）：`GET /api/recording/schedules`、`POST`（upsert；頻道>0、mask 0..127、分鐘 0..1439、end>=start 皆 400 驗證）、`DELETE /{id:long}`（404）。
    - SPA：錄影排程面板——列表（頻道/時段/狀態/刪除）＋新增表單（頻道、週日勾選、時/分）；lib `scheduleLabel`（遮罩→連續範圍、空 span→無）與 `scheduleInEffect`（本地日＋分鐘判定，enabled false 優先）。
    - 測試：Api ＋1 **28**（CRUD＋400 兩分支＋404＋Empty）、vitest ＋1 **31**（label 範圍/無、inEffect 命中/越界/停用）；全量 **1126**。
107. **M129 交付＝智慧看板自動編排（§7.7 smartwall 視界收尾）**：
    - lib `focusLayout(cells, cols=4)` 純邏輯：critical 優先佔 2×2 超格（頂端列起，每列 floor(cols/2) 格），其餘正常格墊於 critical 帶正下方；面積守恆，輸出 `{left,top,width,height}` 分數座標（供 pinStyles 轉 %）。
    - SPA `renderSmartwall`：改以 focusLayout 取代 gridLayout 平鋪（`?key=` WS `alarm.*` 重繪不變）。
    - 測試：vitest ＋3 **30**（雙 critical 放大＋正常格下移、單 critical 錯位對齊、空輸入）；dotnet 不變 **1125**。
106. **M128 交付＝日報 REST＋SPA 視界（§14.1 row 7 日報自動化）**：
    - WebApi（DI 補註冊 ReportRepository；承接 M60 統計報表）：`GET /api/reports/daily?from&to`（缺省＝今日 UTC；from≥to → 400）聚合 `{Recording（各頻道 final 時數/位元組，LEFT JOIN 全渠道列），Capacity（逐日趨勢），Disconnects（offline 筆數），Events（按 event_type 計數 DESC）}`。
    - SPA：日報面板（全角色可見）——錄影時數/GiB/中斷摘要＋事件類別表；`dailyCard` 純邏輯（時數合計、GiB、事件排序）。
    - 測試：Api +1 **27**（歷史固定窗口隔離：一段 1h final＋motion/offline；Recording 2 渠道、Hours>0、Events 含 motion、Disconnect 1、400 分支）＋vitest ＋1 **28**（dailyCard 彙總/GB/排序/空值）；全量 **1125**。
105. **M127 交付＝稽核查詢＋CSV 匯出（§14.1 row 2 稽核匯出工具）**：
    - WebApi（DI 補註冊 AuditLogRepository；承接 M109 audit_log）：`GET /api/audit`（category/actor/action/from/to 多條目篩選、時序 DESC、limit≤500）；`GET /api/audit/export.csv`（同篩選全量、UTF-8 BOM、RFC4180 欄位引號跳脫、Content-Disposition 附件）。from≥to → 400。
    - SPA：admin 限定「稽核日誌」面板——類別下拉＋操作者篩選（`auditRows`/`auditFilter` 純邏輯）＋CSV 下載連結隨篩選同步、表格渲染。
    - 測試：Api +1 **26**（category 篩選、from≥to 400、CSV BOM＋引號含逗號跳脫）＋vitest ＋1 **27**（auditRows 正規化＋auditFilter 類別/操作者）；全量 **1124**。
...
103. **M125 交付＝設定 API＋SPA 設定視界（§14.1 row 20 設定）**：
    ...
    - 測試：Api +1 **25**（預設直／PUT 生效／逾界 400）＋vitest ＋1 **25**（configCard camel/Pascal/缺省）；全量 **1123**。
102. **M124 交付＝帳號管理 CRUD API＋SPA 管理視界（§14.1 row 16 帳號管理）**：
    ...
    - 測試：Api +1 **24**（建立/重複 409/角色 400/清單無 hash/轉角色＋停用/刪除 404）＋vitest ＋1 **24**（accountRows 正規化＋排序）；全量 **1123**。
101. **M123 交付＝登入 API＋SPA 角色門控（§14.1 row 19）**：
    ...
    - 測試：Api +1 **23**（admin 200、viewer 登入需金鑰 401、錯密碼 401、空白 400）＋vitest ＋2 **23**（canAct、parseLogin）；全量 **1123**。
100. **M122 交付＝HeliVms.Web 督導大屏視界（SPA 迭代 2）**：
    ...
    - 測試：vitest ＋9（gridLayout x2、pinStyles x2、smartwallSnapshot x2、ackRate、posTotals、動作 payload）＝**21**；.NET 全量維持 **1123**（本輪無新 API）。
99. **M121 交付＝HeliVms.Web SPA 最小骨架（§14.3 HeliVms.Web）**：
    ...
    - 測試：vitest 30（formatTimestamp/eventRow/排序/KPI/query/WS 訊框/gap）＋Api +3（SPA index 免鑰、lib.js 靜態、WS query-key 握手）；CI 加 Node 24＋`npm ci && npm test`。
...
97. **M119 交付＝回放時間軸 REST（§14.3 播放 REST 真空）**：
    ...
    - 測試 +2（有段有事件回傳 bar＋motion marker＋部分 gap；空日→bars 空／gap 1.0／count 1）＝Api 22；全量 **1115**。
96. **修正（證據匯出範圍整合）**：M119 嘗試之 `EvidenceExporter`＋靜態 `EvidenceSigner` 與 M53 `EvidenceManifestService`/`EvidenceSigner`（RSA-2048、manifest.json＋DB 佐證、OK/TAMPERED/MISSING/EXTRA 驗證）重疊——已完全移除該重複實作並還原 M53 檔案；僅保留 MQTT Hub 測試時限 5s→10s 硬化。證據清冊＋簽章落地以 M53 為準。
    - 回歸全量 **1113**（Storage 782/Api 22/Alarms 254/Devices 54/Licensing 8）。
95. **M118 交付＝WebSocket 警報即時流（§14.3 P0 WebSocket＋#14 訂閱下發）**：
    ...
    - 測試 +2（WS 串流 triage 至 `alarm.triage/high`、無金鑰拒接）＝Api 22；全量 **1113**。
