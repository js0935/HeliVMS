...
## 現況快照（權威來源＝git，非聊天記憶）
- 最後 commit：`HEAD`＝**M17（回放鍵盤快捷鍵）**
  ——`PlaybackWindow` 掛 `PreviewKeyDown`：`Space` 播放/暫停、`←`/`→` **±10 秒**、
  `Shift+←/→` **±10 分**（跨段）、`Ctrl+←/→` **±1 分**（跨段）、`F` 逐幀（約 1/15 秒、
  播一幀後保留暫停）、`Esc` 停止；輸入控制項（ComboBox/DatePicker/Slider 等）聚焦時放行不攔截。
- 前一個 M16 主交付＝`0056ec5`（回放事件列表點擊即跳播至事件時刻、AutomationId `PlaybackEventList`）
- 全部里程碑（M1..M17）依 `docs\ARCHITECTURE.md` 的里程碑定序交付；**roadmap 文件沒有 M18+ 定義**
...
- E2E harness：`C:\Users\JS\AppData\Local\Temp\opencode\e2e\x\Program.cs`
  ...
  - 已有 block：`playcheck`、`playbackcheck`、`playmark`、`plist`（M16，PLIST_OK）、**`kbcheck`（M17，已驗證 KB_OK）**
  - `kbcheck` 驗證：`Space` 暫停（t1）→ `→` 前跳 10 秒（t2−t1＝11s，播放效率差 1s 內）→ `F` 逐幀仍暫停 →
    `Esc` 停止（StopButton disabled）；輸出 `KB_OK`

## 尚未完成／下一步（依 git 與 repo 判斷）
1. **M18 定義（尚未開立）**：roadmap 目前無定義，需先在 `docs` 制定 M18 內容
   （候選：事件中心卡片式檢視——`EventCenterWindow` 事件清單改卡片、展開即有縮圖+詳情；
   §8.5 提及但未實作的其他快捷鍵；錄影排程 UI 校時等），才可照「先定義、後實作」的節奏進行
