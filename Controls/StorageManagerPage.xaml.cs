...
public partial class StorageManagerPage : UserControl {
    ...
    private readonly ObservableCollection<StorageLocation> _locations = [];
    ...
    private void SaveLocations() {
        try {
            var json = JsonSerializer.Serialize(_locations.ToList(), JsonOptions);
    ...
    private void AddPath_Click(object sender, RoutedEventArgs e) {
        ...
        for (var i = 0; i < _locations.Count; i++) {
            if (string.Equals(_locations[i].Path, folder, StringComparison.OrdinalIgnoreCase)) { exists = true; break; }
        }
        if (exists) {
    ...
    private void EditPath_Click(object sender, RoutedEventArgs e) {
        ...
        for (var i = 0; i < _locations.Count; i++) {
            var l = _locations[i];
            if (l != selected && string.Equals(l.Path, folder, StringComparison.OrdinalIgnoreCase)) { exists = true; break; }
        }
        if (exists) {
            MessageBox.Show("該路徑已經存在於列表中", "提示",
                MessageBoxButton.OK, MessageBoxImage.Information);
