...
public partial class ChannelManagementPage : UserControl
{
    ...
    private List<ChannelItem> _channelItems = new();
    private List<Camera> _originalCameras = new();
    ...
    internal void LoadChannels()
    {
        ...
        UpdateChannelCount();
        CurrentChannels = _channelItems.ToList();
    ...
    {
        var channel = ChannelGrid.SelectedItem as ChannelItem;
        var cameraItem = CameraListBox.SelectedItem as CameraListItem;

        if (channel is null || cameraItem?.Camera is null) { return; }
...
$"是否要將 IP 重新指派給頻道 {channel.ChannelNumber}？",
    ...
    private void AutoAssignBtn_Click(object sender, RoutedEventArgs e)
    {
        ...
        unassigned.Sort((a, b) =>
        {
            bool aIs192 = a.IpAddress?.StartsWith("192.") == true;
            bool bIs192 = b.IpAddress?.StartsWith("192.") == true;
            if (aIs192 != bIs192)
                return aIs192 ? -1 : 1;
