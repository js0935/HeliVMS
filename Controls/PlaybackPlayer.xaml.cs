...
public partial class PlaybackPlayer : UserControl {
    // GPU rendering via D3DImage (with WriteableBitmap fallback)
    private D3DImageSurface? _d3dSurface;
    private WriteableBitmap? _writeableBitmap;
    private bool _useGpuRender = true;

    private static readonly SolidColorBrush PlayingBrush = FreezeBrush(new SolidColorBrush(Color.FromRgb(0x4C, 0xAF, 0x50)));
    ...
    private static readonly Brush SelectedBorderBrush = FreezeBrush(new SolidColorBrush(Color.FromArgb(180, 0x4C, 0xAF, 0x50)));
    private static readonly Brush NormalBorderBrush = FreezeBrush(new SolidColorBrush(Color.FromArgb(51, 0x80, 0x80, 0x80)));

    private static T FreezeBrush<T>(T b) where T : Brush { b.Freeze(); return b; }
    ...
    private void OnPreviewMouseWheel(object sender, MouseWheelEventArgs e) {
        ...
        var newZoom = Math.Clamp(_zoomLevel + delta, ZoomMin, ZoomMax);
        if (Math.Abs(newZoom - _zoomLevel) < 0.01) { return; }

        var pos = e.GetPosition(VideoImage);
        var relX = (pos.X / VideoImage.ActualWidth - 0.5) * 2;
        var relY = (pos.Y / VideoImage.ActualHeight - 0.5) * 2;

        var scaleFactor = newZoom / _zoomLevel;
        _zoomLevel = newZoom;
    ...
    private void ClampPanOffset() {
        ...
        var maxX = (VideoImage.ActualWidth * _zoomLevel - VideoImage.ActualWidth) / 2;
        var maxY = (VideoImage.ActualHeight * _zoomLevel - VideoImage.ActualHeight) / 2;
    ...
    public void ZoomToPoint(double zoomFactor, Point center) {
        ...
        ZoomScale.ScaleX = _zoomLevel;
        ZoomScale.ScaleY = _zoomLevel;
        var relX = (center.X / VideoImage.ActualWidth - 0.5) * 2;
        var relY = (center.Y / VideoImage.ActualHeight - 0.5) * 2;
    ...
    private static D3DRenderer? GetD3DRenderer() {
        return D3DRenderer.Instance;
    }

    /// <summary>Display a decoded video frame from pooled buffer</summary>
    public unsafe void ShowFrame(PooledBuffer frame) {
        ...
        if (_useGpuRender) {
            // Try GPU path first
            var renderer = GetD3DRenderer();
            if (renderer != null) {
                if (_d3dSurface == null ||
                    _d3dSurface.Image.PixelWidth != width ||
                    _d3dSurface.Image.PixelHeight != height) {
                    _d3dSurface?.Dispose();
                    _d3dSurface = new D3DImageSurface(renderer);
                    _d3dSurface.EnsureSize(width, height);
                    VideoImage.Source = _d3dSurface.Image;
                }
                ...
                _d3dSurface.PresentFrame(frame.DataPtr, frame.DataSize, stride);
                frame.Dispose();
            ...
            // GPU not available, fall back to WriteableBitmap permanently
            _useGpuRender = false;
            _d3dSurface?.Dispose();
            _d3dSurface = null;
        }
        ...
        try {
            ...
            Buffer.MemoryCopy((void*)frame.DataPtr, (void*)_writeableBitmap.BackBuffer,
                copySize, Math.Min(frame.DataSize, copySize));
    ...
    public string? SaveSnapshot(string directory) {
        ...
        try {
            ...
            var w = _writeableBitmap.PixelWidth;
            var h = _writeableBitmap.PixelHeight;

            // Composite frame + timestamp overlay using DrawingVisual
            var visual = new DrawingVisual();
            var text = $"{ChannelNumber}  {CameraName}  {TimestampText.Text}";
            double barHeight = 26;
            var fontSize = Math.Max(10, Math.Min(16, h * 0.025));
            ...
            using (var dc = visual.RenderOpen()) {
                ...
                var textX = Math.Max(4, (w - ft.Width) / 2);
                var textY = h - barHeight + (barHeight - ft.Height) / 2;
