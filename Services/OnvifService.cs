...
public partial class OnvifService(QCTekService? qctek = null, RtspUrlResolver? urlResolver = null) : IOnvifService {
    private readonly HttpClient _http = new() { Timeout = TimeSpan.FromSeconds(10) };
    private readonly QCTekService? _qctek = qctek;
    private readonly RtspUrlResolver _urlResolver = urlResolver ?? new RtspUrlResolver();
    ...
    public async Task<List<OnvifDiscoveryResult>> ScanSubnetAsync(
        ...
        for (var i = 1; i <= 254; i++) {
            ...
            tasks.Add(Task.Run(async () => {
                try {
                    var (Ip, Result) = await TryDiscoverAsync(ip, onvifPort, username, password).ConfigureAwait(false);
                    return (ip, Result);
                } finally {
                    semaphore.Release();
        ...
        foreach (var (Ip, Result) in allResults)
    ...
    public async Task<OnvifDiscoveryResult?> DiscoverCameraAsync(
        ...
        var profiles = await GetProfilesAsync(ip, port, username, password).ConfigureAwait(false) ?? [];
    ...
    public async Task<OnvifDiscoveryResult?> DiscoverCameraWithPortFallbackAsync(
        ...
        if (preferredPort.HasValue) {
            ...
            var count = 1;
            for (var pi = 0; pi < DefaultOnvifPorts.Length; pi++)
            ...
            if (result is not null) {
                if (result.Profiles.Count > 0) {
                    ...
                    if (profilesWithRtsp.Count > 0) {
                        ...
                        for (var pi = 0; pi < profilesWithRtsp.Count; pi++) {
        ...
        try {
            ...
            if (idx >= 0) {
                ...
                if (xaddrIdx >= 0) {
                    ...
                    if (end > xaddrIdx) {
                        return body[(xaddrIdx + 7)..end].Trim();
                    }
                }
            }
        } catch (Exception ex) when (ex is TaskCanceledException or OperationCanceledException or HttpRequestException) {
    ...
    private static string BuildAuthHeader(string username, string password) {
        ...
        string digest;
        digest = Convert.ToBase64String(SHA1.HashData(raw));
    ...
    public async Task<OnvifPTZStatus?> PTZ_GetStatusAsync(
        ...
        try {
            ...
            foreach (var e in doc.Descendants()) {
                if (e.Name == XName.Get("{http://www.onvif.org/ver20/ptz/wsdl}PTZStatus")) { positionNode = e; break; }
            }
    ...
    public async Task<OnvifPTZConfiguration?> PTZ_GetConfigurationAsync(
        ...
        try {
            ...
            foreach (var e in doc.Descendants()) {
                if (e.Name == XName.Get("{http://www.onvif.org/ver20/ptz/wsdl}PTZConfiguration")) { configNode = e; break; }
            }
    ...
    private async Task<string?> GetPTZServiceUrlAsync(
        ...
        try {
            ...
            foreach (var e in doc.Descendants()) {
                if (e.Name == XName.Get("{http://www.onvif.org/ver10/device/wsdl}PTZ")) { ptzNode = e; break; }
            }
    ...
    private static XDocument SafeParseXml(string xml) {
        ...
        var sanitized = MyRegex().Replace(xml, "&amp;");
    ...
    [GeneratedRegex("&(?!(amp|lt|gt|quot|apos|#[0-9]+|#x[0-9a-fA-F]+);)", RegexOptions.IgnoreCase, "zh-TW")]
