...
public class RtspUrlResolver {
    private readonly List<SubstreamRule> _rules = [];
        ...
        try {
            ...
            if (File.Exists(rulesPath)) {
                ...
                if (doc?.Rules is not null && doc.Rules.Count > 0) {
                    ...
                    for (var ri = 0; ri < doc.Rules.Count; ri++) {
                        var r = doc.Rules[ri];
                        var find = r.Find ?? "";
                ...
                if (doc?.Brands is not null) {
                    foreach (var kvp in doc.Brands) {
                        _brandAliases[kvp.Key] = kvp.Value.Aliases ?? [];
                    }
                }
            }
        } catch (Exception ex) {
    ...
    public string? TryDeriveSubStreamUrl(string mainUrl, string? manufacturer = null) {
        ...
        if (!string.IsNullOrWhiteSpace(manufacturer)) {
            ...
            if (matchedBrand is not null) {
                ...
                for (var i = 0; i < _rules.Count; i++) {
                    ...
                    for (var j = 0; j < r.Brands.Count; j++) {
        ...
        foreach (var rule in _rules) {
            ...
            if (!string.IsNullOrWhiteSpace(manufacturer)) {
                var brandMatch = false;
                for (var i = 0; i < rule.Brands.Count; i++) {
    ...
    private static string? TryApplyRule(string url, SubstreamRule rule) {
        var idx = url.IndexOf(rule.Find, StringComparison.OrdinalIgnoreCase);
        if (idx < 0) { return null; }

        if (rule.Find.StartsWith('/') && !rule.Find.EndsWith('/')) {
            var endIdx = idx + rule.Find.Length;
    ...
    private string? FindMatchingBrand(string manufacturer) {
        ...
        foreach (var (brandName, aliases) in _brandAliases) {
            ...
            if (aliases is not null) {
                for (var i = 0; i < aliases.Count; i++) {
                    if (manufacturer.Contains(aliases[i], StringComparison.OrdinalIgnoreCase)) {
                        aliasMatch = true;
                        break;
                    }
                }
        ...
        foreach (var rule in _rules) {
            foreach (var brand in rule.Brands) {
                if (manufacturer.Contains(brand, StringComparison.OrdinalIgnoreCase))
                    return brand;
            }
        }
    ...
    private void LoadBuiltinRules() {
        ...
        _rules.Add(new SubstreamRule { Id = "hik-live1s", Find = "live1s1", Replace = "live1s2", Brands = ["hikvision"] });
        _rules.Add(new SubstreamRule { Id = "hik-channels-101-102", Find = "/Streaming/Channels/101", Replace = "/Streaming/Channels/102", Brands = ["hikvision"] });
        _rules.Add(new SubstreamRule { Id = "hik-channels-1-2", Find = "/Streaming/Channels/1", Replace = "/Streaming/Channels/2", Brands = ["hikvision"] });
        _rules.Add(new SubstreamRule { Id = "hik-main-sub-av", Find = "/main/", Replace = "/sub/", Brands = ["hikvision"] });
        _rules.Add(new SubstreamRule { Id = "hik-h264-main-sub", Find = "/h264/ch1/main/", Replace = "/h264/ch1/sub/", Brands = ["hikvision"] });
        _rules.Add(new SubstreamRule { Id = "hik-ch0-0-ch0-1", Find = "ch0_0.h264", Replace = "ch0_1.h264", Brands = ["hikvision"] });
        _rules.Add(new SubstreamRule { Id = "dahua-subtype-00-01", Find = "subtype=00", Replace = "subtype=01", Brands = ["dahua"] });
        _rules.Add(new SubstreamRule { Id = "dahua-subtype-0-1", Find = "subtype=0", Replace = "subtype=1", Brands = ["dahua"] });
        _rules.Add(new SubstreamRule { Id = "reolink-main-sub", Find = "_main", Replace = "_sub", Brands = ["reolink"] });
        _rules.Add(new SubstreamRule { Id = "reolink-stream-0-1", Find = "stream=0", Replace = "stream=1", Brands = ["reolink"] });
        _rules.Add(new SubstreamRule { Id = "uniview-video1-video2", Find = "/media/video1", Replace = "/media/video2", Brands = ["uniview"] });
        _rules.Add(new SubstreamRule { Id = "uniview-s0-s1", Find = "/s0/live", Replace = "/s1/live", Brands = ["uniview"] });
        _rules.Add(new SubstreamRule { Id = "aver-live-st1-st2", Find = "live_st1", Replace = "live_st2", Brands = ["aver"] });
        _rules.Add(new SubstreamRule { Id = "generic-stream1-stream2", Find = "stream1", Replace = "stream2", Brands = [] });
        _rules.Add(new SubstreamRule { Id = "generic-stream-main-sub", Find = "stream-main", Replace = "stream-sub", Brands = [] });
        _rules.Add(new SubstreamRule { Id = "generic-profile1-profile2", Find = "profile1", Replace = "profile2", Brands = [] });
        _rules.Add(new SubstreamRule { Id = "generic-onvif1-onvif2", Find = "onvif1", Replace = "onvif2", Brands = [] });
        _rules.Add(new SubstreamRule { Id = "generic-ch1-ch2", Find = "/ch1/", Replace = "/ch2/", Brands = [] });
        _rules.Add(new SubstreamRule { Id = "generic-channel1-channel2", Find = "channel=1", Replace = "channel=2", Brands = [] });
        _rules.Add(new SubstreamRule { Id = "generic-video1-video2", Find = "video1", Replace = "video2", Brands = [] });
        _rules.Add(new SubstreamRule { Id = "generic-av0-av1", Find = "av0", Replace = "av1", Brands = [] });
        _rules.Add(new SubstreamRule { Id = "generic-ucast-1-2", Find = "ucast/1", Replace = "ucast/2", Brands = [] });
        _rules.Add(new SubstreamRule { Id = "generic-main-sub", Find = "/main", Replace = "/sub", Brands = [] });
        _rules.Add(new SubstreamRule { Id = "generic-live1-live2", Find = "/live1", Replace = "/live2", Brands = [] });
        _rules.Add(new SubstreamRule { Id = "generic-high-low-res", Find = "HighResolutionVideo", Replace = "LowResolutionVideo", Brands = [] });
