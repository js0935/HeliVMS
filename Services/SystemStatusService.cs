...
public sealed class SystemStatusService : ISystemStatusService, IDisposable {
    ...
    public SystemStatusService() {
        ...
        _cpuCounter = new Lazy<PerformanceCounter?>(() => {
            try { var pc = new PerformanceCounter("Processor", "% Processor Time", "_Total"); pc.NextValue(); return pc; } catch { return null; }
    ...
    public void StartMonitoring() {
        _timer?.Dispose();
        _timer = new Timer(_ => {
            try { RefreshAll(); } catch (Exception ex) { Log.Warning(ex, "[SystemStatus] Refresh error"); }
