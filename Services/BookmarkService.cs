...
public sealed class BookmarkService : IBookmarkService
{
    ...
    public List<PlaybackBookmark> LoadBookmarks(DateTime date)
    {
        try
        {
            ...
            if (!File.Exists(path)) { return new List<PlaybackBookmark>(); }
            var json = File.ReadAllText(path, System.Text.Encoding.UTF8);
            return JsonSerializer.Deserialize<List<PlaybackBookmark>>(json) ?? new List<PlaybackBookmark>();
        }
        catch (Exception ex)
        {
            Log.Debug("[HeliVMS] BookmarkService load error: {Msg}", ex.Message);
            return new List<PlaybackBookmark>();
        }
    }
