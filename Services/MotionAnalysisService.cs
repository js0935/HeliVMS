...
public sealed class MotionAnalysisService : IMotionAnalysisService, IDisposable
{
    ...
    private static readonly Regex SceneScoreRegex = new(
        @"lavfi\.scene_score=([\d\.]+)",
        RegexOptions.Compiled | RegexOptions.IgnoreCase);

    public MotionAnalysisService(IVideoIndexService videoIndexService, IEventService eventService)
    {
    ...
    public void StopMonitoring()
    {
        ...
        catch (Exception ex) { Log.Warning(ex, "[HeliVMS] MotionAnalysisService stop error"); }
        _monitorTask = null;
    ...
    private async Task AnalyzePendingSegmentsAsync(CancellationToken ct)
    {
        ...
        foreach (var seg in segments)
        {
            ...
            try
            {
                double score = await AnalyzeSingleSegmentAsync(ffmpeg, seg.FilePath, ct)
                    .ConfigureAwait(false);
    ...
    private static async Task<double> AnalyzeSingleSegmentAsync(string ffmpeg, string filePath, CancellationToken ct)
    {
        ...
        var completed = proc.WaitForExit((int)FfmpegTimeout.TotalMilliseconds);
