...
public sealed partial class VideoIndexService : IVideoIndexService, IDisposable {
    ...
    public async Task EnsureDatabaseCreatedAsync() {
        ...
        // Ensure StartTime index for GetDailyRecordingStatsAsync range queries
        await db.Database.ExecuteSqlRawAsync(
            "CREATE INDEX IF NOT EXISTS IX_VideoSegments_StartTime " +
            "ON VideoSegments(StartTime);").ConfigureAwait(false);

        // Ensure IntegrityCheckedAt index for QueryUncheckedSegmentsAsync
        await db.Database.ExecuteSqlRawAsync(
            "CREATE INDEX IF NOT EXISTS IX_VideoSegments_IntegrityCheckedAt " +
            "ON VideoSegments(IntegrityCheckedAt);").ConfigureAwait(false);

        // Ensure composite index for QueryUnanalyzedMotionSegmentsAsync
        await db.Database.ExecuteSqlRawAsync(
            "CREATE INDEX IF NOT EXISTS IX_VideoSegments_MotionAnalysis_EndTime " +
            "ON VideoSegments(MotionAnalysisDone, EndTime DESC);").ConfigureAwait(false);
    ...
    public async Task BulkUpdateEndTimeAsync(IEnumerable<(int SegmentId, DateTime EndTime, long FileSize)> updates) {
        ...
        using var db = CreateDbContext();
        var ids = list.Select(u => u.SegmentId).ToHashSet();
        var segments = await db.VideoSegments
            .Where(s => ids.Contains(s.Id))
            .ToListAsync().ConfigureAwait(false);
        var segLookup = segments.ToDictionary(s => s.Id);
        foreach (var (segId, endTime, fileSize) in list) {
            if (segLookup.TryGetValue(segId, out var segment)) {
                segment.EndTime = endTime;
                segment.FileSize = fileSize;
            }
        }
    ...
    public async Task<int> CleanupOrphanSegmentsAsync() {
        ...
        using var db = CreateDbContext();
        List<VideoSegment> page;
        do {
            page = await db.VideoSegments
                .Where(s => s.Id > lastId)
                .OrderBy(s => s.Id)
                .Take(pageSize)
                .ToListAsync().ConfigureAwait(false);
            ...
            foreach (var seg in page) {
                ...
                if (isRealFile) {
                    ...
                    var orphanSeg = await db.VideoSegments.FindAsync(seg.Id).ConfigureAwait(false);
                    if (orphanSeg is not null) {
                        db.VideoSegments.Remove(orphanSeg);
                        deleted++;
                    }
                }
            }
    ...
    public async Task<(int DeletedSegments, long FreedBytes)> PurgeByRetentionPolicyAsync(
        ...
        if (maxStorageGB > 0) {
            var maxBytes = (long)maxStorageGB * 1024L * 1024L * 1024L;

            // Query current total recording usage
            var currentBytes = await db.VideoSegments.SumAsync(s => (long?)s.FileSize ?? 0)
                .ConfigureAwait(false);

            if (currentBytes > maxBytes) {
                var excess = currentBytes - maxBytes;
    ...
    public async Task UpdateIntegrityStatusAsync(IEnumerable<(int SegmentId, bool IsCorrupted)> updates) {
        await ExecWithRetryAsync(async db => {
            ...
            for (var ui = 0; ui < list.Count; ui++) { ids.Add(list[ui].SegmentId); }
            var segments = await db.VideoSegments
                .Where(s => ids.Contains(s.Id))
                .ToListAsync().ConfigureAwait(false);
    ...
    public async Task<int> BulkUpdateFilePathPrefixAsync(string oldPrefix, string newPrefix) {
        ...
        foreach (var seg in segments) {
            var relative = seg.FilePath[oldPrefix.Length..];
    ...
    public async Task<Dictionary<DateTime, (int SegmentCount, long TotalBytes)>> GetDailyRecordingStatsAsync(
        ...
        return await ExecWithRetryAsync(async db => {
            ...
            var daily = await query
                ...
                .Select(g => new { Date = g.Key, Count = g.Count(), TotalSize = g.Sum(s => s.FileSize) })
                .AsNoTracking()
                .ToListAsync().ConfigureAwait(false);

            var result = new Dictionary<DateTime, (int SegmentCount, long TotalBytes)>(31);
            foreach (var row in daily) {
                result[row.Date] = (row.Count, row.TotalSize);
            }
            return result;
        }).ConfigureAwait(false);
    }
    ...
    [System.Text.RegularExpressions.GeneratedRegex(@"^(.+)_(\d{8})_(\d{6})$")]
