...
public sealed class LicenseService : ILicenseService
{
    ...
    private static readonly object _machineIdLock = new();
    ...
    public bool Activate(string licenseKey)
    {
        try
        {
            ...
            for (int i = 0; i < parts.Length; i++)
            {
                var p = parts[i];
                bool allHex = true;
                for (int ci = 0; ci < p.Length; ci++) { if (!Uri.IsHexDigit(p[ci])) { allHex = false; break; } }
                if (p.Length != 4 || !allHex)
                { invalid = true; break; }
            }
    ...
    {
        var json = JsonSerializer.Serialize(_license, new JsonSerializerOptions
        {
            WriteIndented = true
        });
        return json;
        ...
        try
        {
            ...
            if (parts.Count < 2)
            {
                try
                {
                    ...
                    for (int di = 0; di < allDrives.Length; di++)
                    {
                        var d = allDrives[di];
