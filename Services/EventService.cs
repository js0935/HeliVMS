...
public class EventService : IEventService
{
    ...
    public List<SystemEvent> GetRecentEvents(int count = 200)
    {
        ...
        int takeCount = Math.Min(count, eventsArray.Length);
        var result = new List<SystemEvent>(takeCount);
        for (int i = eventsArray.Length - 1; i >= eventsArray.Length - takeCount; i--)
        {
    ...
    public List<SystemEvent> QueryEvents(string? category = null, string? severity = null, string? keyword = null, int count = 200)
    {
        ...
        int maxCount = Math.Min(count, snapshot.Length);
        var result = new List<SystemEvent>(maxCount);

        for (int i = snapshot.Length - 1; i >= 0 && result.Count < maxCount; i--)
        {
            var e = snapshot[i];
            ...
            if (kw is not null &&
                !e.Message.ToLowerInvariant().Contains(kw) &&
                !e.Source.ToLowerInvariant().Contains(kw) &&
                !e.Detail.ToLowerInvariant().Contains(kw) &&
                !e.User.ToLowerInvariant().Contains(kw)) { continue; }
            result.Add(e);
        }
        return result;
    }
