...
public sealed class CameraHealthService(ICameraService cameraService, IEventService eventLog, IEventRuleService ruleEngine) : ICameraHealthService, IDisposable {
    ...
    public ObservableCollection<CameraHealthItem> HealthItems { get; } = [];
    ...
    private static readonly TimerCallback _timerCallback = static state => {
        ...
        try { self.DispatchSyncCameras(); } catch (Exception ex) { Log.Debug("[HeliVMS] CameraHealthService sync error: {Msg}", ex.Message); }
    };
