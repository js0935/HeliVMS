...
public sealed partial class RecordingService : IRecordingService, IDisposable {
    ...
    private readonly ConcurrentDictionary<string, SegmentRecorder> _activeRecorders = new();
    ...
    private readonly Timer? _tsPollTimer;
    private const int SoftwareEncodingLimit = 8;
    private const int TsPollIntervalMs = 15_000;
    ...
    public bool UseInProcessRecorder { get; set; }
    ...
    public RecordingService(IEventService eventLog, ISettingsService settingsService, IVideoIndexService videoIndexService) {
        ...
        // Poll for new .ts files every 15s instead of FileSystemWatcher (64 FSW overflow risk)
        _tsPollTimer = new Timer(async _ => await PollNewTsFilesAsync(), null, TsPollIntervalMs, TsPollIntervalMs);
    }
        ...
        try {
            if (File.Exists(StorageConfigPath)) {
                ...
                if (locations is not null) {
                    for (var li = 0; li < locations.Count; li++) {
                        var l = locations[li];
                        if (l.IsActive && !string.IsNullOrWhiteSpace(l.Path)) { active = l; break; }
                    }
                }
    ...
    public bool StartRecording(Camera camera) {
        ...
        if (UseInProcessRecorder) {
            return StartInProcessRecording(camera, dir, outputPattern);
        }
        ...
        if (_activeRecordings.TryAdd(camera.Id, session)) {
            ...
            // Bootstrap: index any .ts files created between camera start and poll cycle
            _ = BootstrapExistingTsFilesAsync(camera.Id, dir, session.StartedAt, recordType);
    ...
    private bool StartInProcessRecording(Camera camera, string dir, string outputPattern) {
        var recorder = new SegmentRecorder(camera.RtspUrl, outputPattern, camera.Id, _segmentLengthSeconds) {
            UseAsyncRtsp = _settingsService.Settings.UseAsyncRtspRecording
        };
        recorder.SegmentCreated += (filePath) => {
            var fileName = Path.GetFileName(filePath);
            var processed = _processedTsFiles.GetOrAdd(camera.Id, _ => new HashSet<string>(StringComparer.OrdinalIgnoreCase));
            lock (processed) {
            ...
            if (segmentStart < DateTime.Now.AddMinutes(-1))
                segmentStart = DateTime.Now.AddMinutes(-1);
            ...
            var segment = new VideoSegment {
                ...
                EndTime = fileTime,
                FilePath = filePath,
                RecordType = GetRecordTypeFromCamera(camera),
            };
            _ = AddIndexSegmentAsync(camera.Id, segment);
        };
        recorder.ErrorOccurred += (msg) => {
            _eventLog.LogWarning(EventCategories.System, "RecordingService",
                $"錄影異常: {camera.Name}", msg);
            var session = _activeRecordings.Values.FirstOrDefault(s => s.CameraId == camera.Id);
            if (session is not null) {
                RecordingStatusChanged?.Invoke(camera.Id, false);
                _ = FinalizeAllSegmentsAsync(camera.Id, session.FilePath);
            }
        };
        ...
        if (_activeRecordings.TryAdd(camera.Id, session) && _activeRecorders.TryAdd(camera.Id, recorder)) {
            recorder.Start();
            RecordingStatusChanged?.Invoke(camera.Id, true);
            _eventLog.LogInfo(EventCategories.Operation, "RecordingService",
                $"Started in-process recording: {camera.Name}", $"Path: {dir}");
            Notify($"錄影開始: {camera.Name}", "INFO");
            return true;
        }

        recorder.Dispose();
        _activeRecorders.TryRemove(camera.Id, out _);
        return false;
    }

    public bool StopRecording(string cameraId) {
        // Sync fallback (kept for interface compliance)
        return StopRecordingAsync(cameraId).GetAwaiter().GetResult();
    }

    public async Task<bool> StopRecordingAsync(string cameraId) {
        if (!_activeRecordings.TryRemove(cameraId, out var session))
            return false;
        ClearRecordingSnapshot();

        // Stop in-process recorder if active
        if (_activeRecorders.TryRemove(cameraId, out var recorder)) {
            recorder.Stop();
            recorder.Dispose();
            _processedTsFiles.TryRemove(cameraId, out _);
            RecordingStatusChanged?.Invoke(cameraId, false);
            _eventLog.LogInfo(EventCategories.Operation, "RecordingService",
                $"Stopped in-process recording: {session.CameraName}",
                $"Duration: {(DateTime.Now - session.StartedAt).TotalMinutes:F1} min");
            Notify($"錄影結束: {session.CameraName}", "INFO");
            _ = FinalizeAllSegmentsAsync(cameraId, session.FilePath);
            return true;
        }

        // Clean up processed file tracking (no FSW to dispose)
        _processedTsFiles.TryRemove(cameraId, out _);
        ...
        if (process is not null) {
            try {
                if (process.HasExited) {
                    ...
                    var exited = await WaitForExitAsync(process, 2000).ConfigureAwait(false);
                    if (!exited) {
    ...
    private static readonly SemaphoreSlim _stopThrottle = new(4, 4);

    public async Task StopAllAsync(Action<string>? onProgress = null) {
        ...
        for (var i = 0; i < ids.Count; i++) {
            var id = ids[i];
            tasks[i] = StopOneAsync(id, onProgress);
        }
        await Task.WhenAll(tasks).ConfigureAwait(false);

        await KillAllFfmpegProcessesAsync().ConfigureAwait(false);
        onProgress?.Invoke("All ffmpeg processes terminated");
    }

    private async Task StopOneAsync(string id, Action<string>? onProgress) {
        await _stopThrottle.WaitAsync().ConfigureAwait(false);
        try {
            var cameraName = _activeRecordings.TryGetValue(id, out var s) ? s.CameraName : id;
            onProgress?.Invoke($"Stopping {cameraName}");
            await StopRecordingAsync(id).ConfigureAwait(false);
        } finally { _stopThrottle.Release(); }
    }

    public void StopAll(Action<string>? onProgress = null) {
        // Sync wrapper for Dispose — fire-and-forget is acceptable during cleanup
        _ = StopAllAsync(onProgress);
    }

    private static readonly SemaphoreSlim _killThrottle = new(4, 4);

    /// <summary>Kill all ffmpeg.exe processes owned by this application</summary>
    public static async Task KillAllFfmpegProcessesAsync() {
        try {
            ...
            for (var pi = 0; pi < processes.Length; pi++) {
                var proc = processes[pi];
                tasks[pi] = KillOneProcessAsync(proc);
            }
            await Task.WhenAll(tasks).ConfigureAwait(false);
        } catch (Exception ex) {
    ...
    private static async Task KillOneProcessAsync(Process proc) {
        await _killThrottle.WaitAsync().ConfigureAwait(false);
        try {
            proc.Kill();
            if (!await WaitForExitAsync(proc, 3000).ConfigureAwait(false)) {
                Log.Debug("[HeliVMS] KillAllFfmpegProcesses: timeout waiting for ffmpeg PID {Pid} to exit", proc.Id);
            }
            proc.Dispose();
        } catch (Exception ex) {
            Log.Debug("[HeliVMS] KillAllFfmpegProcesses: error killing ffmpeg PID {Pid}: {Msg}", proc.Id, ex.Message);
        } finally {
            _killThrottle.Release();
        }
    }
    ...
    public static void KillAllFfmpegProcesses() {
        ...
        try {
            KillAllFfmpegProcessesAsync().GetAwaiter().GetResult();
        } catch { }
    }
    ...
    private List<RecordingSession>? _recordingSnapshot;
    private DateTime _snapshotTime = DateTime.MinValue;
    private static readonly TimeSpan SnapshotMaxAge = TimeSpan.FromMilliseconds(500);

    public List<RecordingSession> GetActiveRecordings() {
        var now = DateTime.UtcNow;
        if (_recordingSnapshot is not null && (now - _snapshotTime) < SnapshotMaxAge) {
        ...
        _recordingSnapshot = list;
        _snapshotTime = now;
        return list;
    }

    public void ClearRecordingSnapshot() {
    ...
    public int GetActiveRecordingCount() {
        var now = DateTime.UtcNow;
        if (_recordingSnapshot is not null && (now - _snapshotTime) < SnapshotMaxAge) {
            return _recordingSnapshot.Count;
        }
    ...
    public void Dispose() {
        _purgeTimer?.Dispose();
        _tsPollTimer?.Dispose();
        try { StopAll(); } catch (Exception ex) { Log.Warning(ex, "[HeliVMS] RecordingService Dispose StopAll error"); }
    }

    private static void Notify(string message, string severity = "WARN") {
        try { App.Services.GetRequiredService<INotificationService>().Show(message, severity); } catch { }
    }
    ...
    private string BuildFfmpegCommand(Camera camera, int segmentSeconds, string outputPattern) {
        ...
        var enableOverlay = _activeRecordings.Count < SoftwareEncodingLimit;

        if (enableOverlay && _settingsService.Settings.EnableCameraNameOverlay) {
        ...
        if (enableOverlay && _settingsService.Settings.EnableTimestampOverlay) {
    ...
    private static readonly Regex TsSegmentPattern = MyRegex();
    ...
    private async Task AddIndexSegmentAsync(string cameraId, VideoSegment segment) {
        try {
            ...
            var list = _segmentIds.GetOrAdd(cameraId, _ => []);
            lock (list)
    ...
    private async Task PollNewTsFilesAsync() {
        try {
            var now = DateTime.Now;
            foreach (var (cameraId, session) in _activeRecordings) {
                if (!_processedTsFiles.TryGetValue(cameraId, out var processed))
                    continue;

                var dir = session.FilePath;
                if (!Directory.Exists(dir)) continue;

                string[] files;
                try { files = Directory.GetFiles(dir, "*.ts"); } catch { continue; }
                ...
                foreach (var file in files) {
                    ...
                    var segmentStart = fileTime.AddSeconds(-_segmentLengthSeconds);
                    if (segmentStart < session.StartedAt)
                        segmentStart = session.StartedAt;
                    ...
                    await AddIndexSegmentAsync(cameraId, segment).ConfigureAwait(false);
                }
            }
        } catch (Exception ex) {
            Log.Debug("[HeliVMS] RecordingService: PollNewTsFiles error: {Msg}", ex.Message);
        }
    }

    private async Task BootstrapExistingTsFilesAsync(string cameraId, string dir, DateTime sessionStart, int recordType) {
        try {
            if (!Directory.Exists(dir)) return;
            string[] files;
            try { files = Directory.GetFiles(dir, "*.ts"); } catch { return; }
            ...
            foreach (var file in files) {
                ...
                OnNewTsFileDetected(cameraId, file, sessionStart, recordType);
            }
        } catch (Exception ex) {
            Log.Debug("[HeliVMS] RecordingService: Bootstrap error: {Msg}", ex.Message);
        }
    }
    ...
    [GeneratedRegex(@"\.ts$", RegexOptions.Compiled)]
