...
public partial class DashboardView : UserControl
{
    ...
    private async void IntegrityCheckBtn_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            var result = await Task.Run(async () => await _videoIndex.RebuildRecordingIndexAsync(storagePath));
    ...
    private async Task RefreshCalendarAsync()
    {
        try
        {
            ...
            int firstDayOfWeek = (int)_calendarMonth.DayOfWeek;
            ...
            {
                // Week number
                int weekNum = GetWeekOfMonth(_calendarMonth.Year, _calendarMonth.Month, r, firstDayOfWeek);
                ...
                for (int c = 0; c < 7; c++)
                {
                    int dayNum = cellIdx - firstDayOfWeek + 1;
                    ...
                    if (dayNum >= 1 && dayNum <= daysInMonth)
                    {
                        ...
                        bool hasRecording = dailyData.TryGetValue(dayDate, out var dayStats);
                        bool hasAllCameras = allCams.Count > 0;

                        // Background: recording presence
                        if (hasRecording)
                        {
                            double density = Math.Min(1.0, dayStats.SegmentCount / (double)(allCams.Count * 24));
                            byte green = (byte)(80 + (int)(density * 120));
    ...
    private static int GetWeekOfMonth(int year, int month, int row, int firstDayOfWeek)
    {
        ...
        int firstDateOfRow = row * 7 - firstDayOfWeek + 1;
