...
public partial class SettingsView : UserControl
{
    ...
    private void ValidateAndImportFfmpeg(string sourcePath)
    {
    ...
    private void UpdateCurrentStorageInfo()
    {
        try
        {
            ...
            if (root is not null)
            {
                ...
                if (drive.IsReady)
                {
                    double usedGB = (drive.TotalSize - drive.AvailableFreeSpace) / (1024.0 * 1024.0 * 1024.0);
    ...
    private MediaMTXService? GetMediaMTX()
    {
        ...
        {
            try { _mtxService = App.Services.GetRequiredService<MediaMTXService>(); }
            catch { }
        }
        return _mtxService;
    }
