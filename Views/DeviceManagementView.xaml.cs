...
public partial class DeviceManagementView : UserControl
{
    ...
    private void ApplyFilters(List<Camera> allCameras)
    {
        ...
        foreach (var c in allCameras)
        {
            if (!string.IsNullOrEmpty(searchText) &&
                !(c.Name?.ToLowerInvariant().Contains(searchText) ?? false) &&
    ...
    private async Task CheckConnectivityAsync()
    {
        ...
        for (int ci = 0; ci < withIp.Count; ci++)
        {
            var c = withIp[ci];
...
var qctek = App.Services.GetService<QCTekService>();
            ...
            foreach (var result in results)
            {
                ...
                string? onvifMainUrl = mainProfile is not null
...
file sealed class IpComparer : IComparer
{
    private readonly ListSortDirection _direction;

    public IpComparer(ListSortDirection dir) => _direction = dir;

    public int Compare(object? a, object? b)
    {
        var ipA = TryParseIp(GetIp(a));
        var ipB = TryParseIp(GetIp(b));
        int result = Comparer<long>.Default.Compare(ipA, ipB);
